...
describe('pipelines + CI sources', () => {
  ...
  it('drift + health runs and workflow templates', async () => {
    ...
    let [url, config] = get.mock.calls.at(-1) as [string, { params: unknown; responseType: string }]
    ...
    ;[url, config] = get.mock.calls.at(-1) as [string, { params: unknown; responseType: string }]
