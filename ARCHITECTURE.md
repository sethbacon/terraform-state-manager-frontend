...
- **Base URL**: empty string. The SPA is same-origin with the backend in every environment — Vite proxies `/api`, `/health`, `/ready`, and `/swagger` to the backend in dev (`vite.config.ts`), and nginx proxies the same paths in production (`nginx.conf.template`). There is **no `VITE_API_URL`** and **no mock-data mode**.
